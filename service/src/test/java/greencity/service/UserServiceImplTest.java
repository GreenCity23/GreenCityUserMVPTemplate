...
class UserServiceImplTest {
    ...
    void updateUserStatusDeactivatedTest() {
        ...
        when(modelMapper.map(userVO, User.class)).thenReturn(user);
        when(modelMapper.map(userVO.getLanguageVO(), Language.class)).thenReturn(ModelUtils.getLanguage());
        assertEquals(DEACTIVATED, userService.updateStatus(userId, DEACTIVATED, userVO2.getEmail()).getUserStatus());
    }
    ...
    void updateUserStatusLowRoleLevelException() {
        ...
        when(modelMapper.map(user, UserVO.class)).thenReturn(userVO);
        assertThrows(LowRoleLevelException.class, () -> userService.updateStatus(userId, DEACTIVATED, user.getEmail()));
    }
    ...
    void getEmailStatusesTest() {
        EmailNotification expectedEmailNotification = EmailNotification.DISABLED;
        when(userRepo.findByEmail(any())).thenReturn(Optional.of(user2));
        assertEquals(expectedEmailNotification, userService.getEmailNotificationsStatuses(userEmail));
    }
    ...
    @Test
    void updateUserProfilePictureNotUpdatedExceptionTest2() {
